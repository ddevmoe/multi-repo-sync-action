...
def _handle_sync_reports(reports: list[RepositoryPathSyncReport]) -> None:
    ...
    logger.info(f"Found {len(reports)} syncs to apply:\n{pretty_diffs}")
    ...
    if config.dry_run:
        logger.warning("Dry run enabled; Doing nothing!")
        return
...
def main() -> None:
    ...
    logger.info(f"Selected {len(target_repositories)} repositories:\n{target_names}")
